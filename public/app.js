...
async function api(key){const r=await fetch(`/api/series/${key}`);if(!r.ok)throw new Error(`${r.status}`);return(await r.json()).observations}
async function apiMulti(keys){const r=await fetch(`/api/multi?series=${keys.join(',')}`);if(!r.ok)throw new Error(`${r.status}`);return r.json()}
