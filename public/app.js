...
const SERIES_IDS={global_simple:'WUIGLOBALSMPAVG',advanced:'WUIADVECON',emerging:'WUIEMERGE',europe:'WUIEUROPE',asia_pacific:'WUIASIAPACIFIC',africa:'WUIAFRICA',middle_east:'WUIMIDEAST',latin_america:'WUIWEST',us:'WUIUSA',china:'WUICHN',uk:'WUIGBR',germany:'WUIDEU',japan:'WUIJPN',india:'WUIIND',brazil:'WUIBRA',france:'WUIFRA',canada:'WUICAN',australia:'WUIAUS',russia:'WUIRUS',south_korea:'WUIKOR',mexico:'WUIMEX',italy:'WUIITA',spain:'WUIESP',saudi_arabia:'WUISAU',turkey:'WUITUR',south_africa:'WUIZAF',nigeria:'WUINGA',egypt:'WUIEGY',argentina:'WUIARG',epu_us_daily:'USEPUINDXD',epu_us_monthly:'USEPUINDXM',epu_global:'GEPUCURRENT',emv_daily:'WLEMUINDXD'};
...
async function api(key){
  ...
  const r=await fetch(`/api/data?key=${sid}`);
  if(!r.ok)throw new Error(r.status);
  return(await r.json()).observations;
}
async function apiMulti(keys){
  const sids=keys.map(k=>SERIES_IDS[k]||k);
  const r=await fetch(`/api/multi?series=${sids.join(',')}`);
  if(!r.ok)throw new Error(r.status);
  const raw=await r.json();
  const out={};keys.forEach(k=>{const sid=SERIES_IDS[k]||k;if(raw[sid])out[k]=raw[sid]});
  return out;
}
